...
PARAM <- list()
PARAM$experimento  <- "CLU1262_local00"
...
boxplot(cliente_edad ~ cluster2,data = dataset)
...
class(dataset)
class(dataset$cluster2)

dataset$cluster2 <- factor(x = dataset$cluster2)

dataset %>%
  dplyr::select(cliente_edad, cliente_antiguedad, internet, Visa_fechaalta) %>%# desestimamos algunas variables
  ggpairs(.,
          mapping = aes(colour = dataset$cluster2, alpha = 0.5),
...
qqplot(cliente_edad ~ cluster2, data = dataset)
