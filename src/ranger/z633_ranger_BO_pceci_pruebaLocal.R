...
kBO_iter  <- 5   #Bajo cantidad para test local
...
#           makeIntegerParam("mtry" ,             lower=    2L, upper=   50L))
            ...
            makeIntegerParam("num.trees" ,        lower=  1L, upper= 5L),       #Bajo cantidad para test local
            makeIntegerParam("max.depth",         lower=    1L, upper=   3L),
            makeIntegerParam("min.node.size" ,    lower=    1L, upper=  500L),
            makeIntegerParam("mtry" ,             lower=    2L, upper=   50L))
...
setwd("/dmef")   #Establezco el Working Directory PARA TEST LOCAL

#cargo el dataset donde voy a entrenar el modelo
#dataset  <- fread("./datasets/competencia2_2022.csv.gz", stringsAsFactors= TRUE)   #donde entreno
dataset  <- fread("./exp/FE7110/dataset_7110.csv.gz", stringsAsFactors= TRUE)   #PARA TEST LOCAL - donde entreno
